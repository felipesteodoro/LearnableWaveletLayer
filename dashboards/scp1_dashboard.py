"""
Streamlit dashboard — SelfRegulationSCP1 Experiment (binary classification, EEG)
...
SCP1_DIR     = Path(__file__).parent.parent / "tests" / "scp1"
RESULTS_BASE = SCP1_DIR / "results"

st.set_page_config(
    page_title="SelfRegulationSCP1 Experiment",
    page_icon="🧠",
    layout="wide",
)
...
def load_exp_config() -> dict:
    try:
        import sys
        cfg_path = str(SCP1_DIR / "config")
        if cfg_path not in sys.path:
            sys.path.insert(0, cfg_path)
        from experiment_config import SCP1_CONFIG, DL_TRAINING_CONFIG, LEARNED_WAVELET_CONFIG
        return {"dataset": SCP1_CONFIG, "training": DL_TRAINING_CONFIG, "wavelet": LEARNED_WAVELET_CONFIG}
    except Exception:
        return {}
...
st.sidebar.title("🧠 SelfRegulationSCP1")
runs = list_runs()

if not runs:
    st.warning("Nenhuma run encontrada. Execute `run_dl_queue.py` em `tests/scp1/` primeiro.")
    st.stop()
...
metric_col   = st.sidebar.selectbox(
    "Métrica principal",
    ["test_accuracy", "test_f1", "test_auc_roc", "test_f1_macro"],
    index=0,
)
...
st.title("🧠 SelfRegulationSCP1 Experiment Dashboard")
st.caption(f"Run: **{run_id}**  |  classificação binária EEG (6 canais, negativity/positivity)  |  "
...
for col in ["test_accuracy","test_f1","test_auc_roc","test_f1_macro",
            "val_accuracy","val_f1","train_accuracy","epochs_trained"]:
...
multi_metrics = [c for c in ["test_accuracy","test_f1","test_auc_roc"] if c in df.columns]
...
display_cols = ["model_name","mode","config_idx","test_accuracy","test_f1",
                "test_auc_roc","val_accuracy","val_f1","epochs_trained"]
