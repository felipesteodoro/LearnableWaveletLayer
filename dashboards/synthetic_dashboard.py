"""
Streamlit dashboard — Synthetic Experiment

Run:
    streamlit run dashboards/synthetic_dashboard.py
"""
import json
import time
...
# Paths
# ---------------------------------------------------------------------------
SYNTHETIC_DIR = Path(__file__).parent.parent / "tests" / "synthetic"
RESULTS_BASE  = SYNTHETIC_DIR / "results"

st.set_page_config(
    page_title="Synthetic Experiment",
    page_icon="🌊",
    layout="wide",
)
...
st.sidebar.title("🌊 Synthetic Experiment")
runs = list_runs()

if not runs:
    st.warning("Nenhuma run encontrada em `tests/synthetic/results/`. Execute `run_dl_queue.py` primeiro.")
    st.stop()
...
st.title("🌊 Synthetic Experiment Dashboard")
