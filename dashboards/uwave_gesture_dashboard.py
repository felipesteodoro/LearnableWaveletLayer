"""
Streamlit dashboard — UWaveGestureLibrary Experiment (multiclass classification)
...
UWAVE_DIR    = Path(__file__).parent.parent / "tests" / "uwave-gesture"
RESULTS_BASE = UWAVE_DIR / "results"

st.set_page_config(
    page_title="UWaveGestureLibrary Experiment",
    page_icon="👋",
    layout="wide",
)
...
def load_exp_config() -> dict:
    try:
        import sys
        cfg_path = str(UWAVE_DIR / "config")
        if cfg_path not in sys.path:
            sys.path.insert(0, cfg_path)
        from experiment_config import UWAVE_CONFIG, DL_TRAINING_CONFIG, LEARNED_WAVELET_CONFIG
        return {"dataset": UWAVE_CONFIG, "training": DL_TRAINING_CONFIG, "wavelet": LEARNED_WAVELET_CONFIG}
    except Exception:
        return {}
...
st.sidebar.title("👋 UWaveGestureLibrary")
runs = list_runs()

if not runs:
    st.warning("Nenhuma run encontrada. Execute `run_dl_queue.py` em `tests/uwave-gesture/` primeiro.")
    st.stop()
...
metric_col   = st.sidebar.selectbox(
    "Métrica principal",
    ["test_accuracy", "test_f1_macro", "test_auc_ovr", "test_f1_weighted"],
    index=0,
)
...
st.title("👋 UWaveGestureLibrary Experiment Dashboard")
st.caption(f"Run: **{run_id}**  |  classificação multivariada multiclasse (8 gestos, 3 canais)  |  "
...
for col in ["test_accuracy","test_f1_macro","test_auc_ovr","test_f1_weighted",
...
pivot = pivot.reindex(index=[m for m in model_order if m in pivot.index],
                      columns=[m for m in mode_order if m in pivot.columns])
...
multi_metrics = [c for c in ["test_accuracy","test_f1_macro","test_auc_ovr"] if c in df.columns]
...
if "test_accuracy" in df.columns and "test_f1_macro" in df.columns:
    st.subheader("Accuracy vs F1-macro")
    fig_sc = px.scatter(
        df, x="test_accuracy", y="test_f1_macro",
...
display_cols = ["model_name","mode","config_idx","test_accuracy","test_f1_macro",
                "test_auc_ovr","val_accuracy","val_f1_macro","epochs_trained"]
