"""
Subprocess entry point — one process per GPU worker (Ford-A experiment).
...
FORDA_DIR = Path(__file__).parent.parent
RESULTS_DIR = Path(os.environ.get("RESULTS_DIR", str(FORDA_DIR / "results")))
...
def _run_experiment(model_name: str, mode: str, config: dict, config_idx: int) -> None:
    sys.path.insert(0, str(FORDA_DIR))

    from src.pipeline_queue import FordAExperimentPipeline

    pipeline = FordAExperimentPipeline(
        model_name=model_name,
        mode=mode,
        config=config,
        config_idx=config_idx,
        results_dir=str(RESULTS_DIR),
        data_dir=str(FORDA_DIR / "data"),
    )
    pipeline.run()
