"""
Launch all DL Ford-A experiments across multiple GPUs.
...
def main():
    parser = argparse.ArgumentParser(description="Ford-A GPU DL experiment queue")
    ...
    if args.fresh:
        run_id, results_dir, mode_label = (
            _new_run_id(), f"results/{_new_run_id()}", f"NOVA RUN → {_new_run_id()}"
        )
    elif args.run_id:
        run_id = args.run_id
    ...
    print(
        f"\n{'='*62}\n"
        f"  Ford-A GPU Job Queue  |  {mode_label}\n"
        f"{'='*62}\n"
