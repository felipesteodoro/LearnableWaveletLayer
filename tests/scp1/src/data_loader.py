"""
Loader para o dataset SelfRegulationSCP1 do UEA Time Series Classification Archive.

Dataset MULTIVARIADO BINÁRIO: 6 canais EEG (slow cortical potentials),
comprimento 896, 2 classes ('negativity'/'positivity'). Faz download (via aeon,
com fallback de download direto do ZIP), mapeia labels (strings → 0/1) e cria
split estratificado de validação.

IMPORTANTE: mantém a dimensão de canais. aeon retorna (n, n_channels, length);
transpomos para (n, length, n_channels) — convenção (batch, time, features)
esperada por base_models.build_model.
"""
import numpy as np
...
class SCP1DataLoader:
    """
    Carrega e prepara o dataset SelfRegulationSCP1 para classificação
    binária multivariada (2 classes, 6 canais EEG).
    """

    # URL direta para download (UEA Archive via timeseriesclassification.com)
    _BASE_URL = (
        "https://www.timeseriesclassification.com/aeon-toolkit/SelfRegulationSCP1.zip"
    )
    ...
    def download(self, force: bool = False) -> Tuple[np.ndarray, np.ndarray,
        ...
        """
        Baixa SelfRegulationSCP1 e retorna (X_train, y_train, X_test, y_test).

        X tem shape (n, length, n_channels) = (n, 896, 6).
        y são labels originais como strings ('negativity'/'positivity').
    ...
    def _parse_ts_file(filepath: Path) -> Tuple[np.ndarray, np.ndarray]:
        ...
        for line in lines[data_start:]:
            ...
            arr = np.array(channels, dtype=np.float32).T   # (length, n_channels)
            X_list.append(arr)
            y_list.append(label)
    ...
    def prepare(
        ...
        # Mapear labels (strings) → inteiros 0..n_classes-1 (binário: 0/1).
        # np.unique ordena de forma estável: 'negativity'→0, 'positivity'→1.
