...
BINÁRIA (SelfRegulationSCP1 — negativity/positivity).
"""
import numpy as np
import pandas as pd
...
class ClassificationEvaluator:
    ...
    Métricas: accuracy, f1 (binária e macro), precision/recall, auc_roc,
    log_loss e specificity (da matriz 2×2).
    """
    ...
    def evaluate(
        ...
        Args:
            ...
            y_prob: probabilidade da classe positiva (para AUC-ROC / log_loss)
        ...
        if y_prob is not None:
            try:
                ...
                results[_k("auc_roc")] = float("nan")
            try:
                results[_k("log_loss")] = float(log_loss(y_true, y_prob))
            except ValueError:
                results[_k("log_loss")] = float("nan")
        else:
            results[_k("auc_roc")] = float("nan")
            results[_k("log_loss")] = float("nan")
    ...
    @staticmethod
    def confusion(y_true: np.ndarray, y_pred: np.ndarray, n_classes: int = 2) -> np.ndarray:
