...
(SelfRegulationSCP1 — multivariate BINARY classification, 2 classes, 6 EEG channels).
"""
...
class SCP1ExperimentPipeline:
    """
    Loads pre-split SelfRegulationSCP1 data, trains one (model_name, mode,
    config_idx) binary classifier, and saves metrics.json.
    ...
    def _load_data(self):
        ...
        def _load(name):
            ...
            if not p.exists():
                raise FileNotFoundError(
                    ...
                    "Run notebook 00_download_and_eda.ipynb first."
                )
            return np.load(p)
        ...
        # Ensure 3-D (batch, time, features). SCP1 já é multivariado (n, 896, 6),
    ...
    def _evaluate(self, model, X_train, y_train, X_val, y_val, X_test, y_test, history):
        ...
        def _metrics(y_true, y_proba, prefix):
            # y_proba: (n, 1) sigmoid → limiar 0.5 para o rótulo predito
            ...
            try:
                out[f"{prefix}_auc_roc"] = float(roc_auc_score(y_true, y_proba))
            except ValueError:
                out[f"{prefix}_auc_roc"] = float("nan")
            return out
