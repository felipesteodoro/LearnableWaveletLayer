"""
GPU Queue Monitor — Rich terminal dashboard (Synthetic-Multivariate experiment).

Run in a separate terminal while the queue is executing:

    python tests/synthetic-multivariate/gpu_queue/dashboard.py
    python tests/synthetic-multivariate/gpu_queue/dashboard.py --status results/2026-05-09/queue_status.json
"""
...
def _render_header(data: dict) -> Panel:
    ...
    return Panel(row, title="[bold blue]Synthetic-Multivariate GPU Job Queue Monitor[/bold blue]", border_style="blue")
...
def _render_config_summary() -> Panel:
    ...
    row1.append(f"n_samples={sig.get('n_samples', '?')}  "
                ...
                f"channels={mv.get('n_channels', '?')}  "
                f"horizon={mv.get('horizon', '?')}  "
                f"weights={mv.get('target_weights', '?')}")
...
def _waiting_panel() -> Panel:
    return Panel(
        ...
        title="[bold blue]Synthetic-Multivariate GPU Job Queue Monitor[/bold blue]",
        border_style="blue",
    )
...
def main():
    parser = argparse.ArgumentParser(description="Synthetic-Multivariate GPU Job Queue Monitor")
