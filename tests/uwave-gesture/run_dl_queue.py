"""
Launch all DL UWaveGestureLibrary experiments across multiple GPUs.

Usage:
    cd tests/uwave-gesture
    python run_dl_queue.py --fresh       # nova run
    python run_dl_queue.py               # auto-resume
    python run_dl_queue.py --run-id 2026-05-29
...
def main():
    parser = argparse.ArgumentParser(description="UWaveGestureLibrary GPU DL experiment queue")
    ...
    print(
        f"\n{'='*62}\n"
        f"  UWaveGestureLibrary GPU Job Queue  |  {mode_label}\n"
        f"{'='*62}\n"
