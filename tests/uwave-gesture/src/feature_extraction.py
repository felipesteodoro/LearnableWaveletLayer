...
class WaveletFeatureExtractor:
    ...
    def extract_features(self, X: np.ndarray) -> np.ndarray:
        """
        Extrai features de múltiplos sinais (uni ou multivariados).

        Args:
            X: (n_samples, sequence_length) — univariado
               (n_samples, sequence_length, n_channels) — multivariado:
               decompõe cada canal e concatena as features ao longo do eixo final.

        Returns:
            np.ndarray: Matriz de features (n_samples, n_features[*n_channels])
        """
        if X.ndim == 3:
            # Multivariado: features por canal, concatenadas.
            n_channels = X.shape[2]
            per_channel = [
                np.array([self.extract_features_single(X[i, :, c]) for i in range(len(X))])
    ...
    def get_multilevel_coefficients_multivariate(
        self,
        X: np.ndarray,
        align_length: bool = True,
    ) -> np.ndarray:
        """
        Versão multivariada de get_multilevel_coefficients.

        Aplica a decomposição por canal e concatena os coeficientes
        (approx + detalhes) de todos os canais ao longo do eixo final.

        Args:
            X: (n_samples, sequence_length, n_channels)
            align_length: alinha comprimentos via interpolação

        Returns:
            np.ndarray: (n_samples, max_length, n_channels * (level + 1))
        """
        if X.ndim == 2:
            return self.get_multilevel_coefficients(X, align_length=align_length)
        if X.ndim != 3:
            raise ValueError(
                f"Esperado X com 2 ou 3 dimensões, recebeu shape={X.shape}"
            )

        n_samples, _, n_channels = X.shape
        per_channel = [
            self.get_multilevel_coefficients(X[..., c], align_length=align_length)
            for c in range(n_channels)
        ]
        # cada elemento: (n_samples, max_len, level+1)
        return np.concatenate(per_channel, axis=-1)
