"""
Model factories for the UWaveGestureLibrary experiment (multiclass classification).

DL models delegate to models/base_models.py — all backbone implementations
live there so that synthetic, financial, ford-a and uwave share the same code.
ML (scikit-learn) classifiers are experiment-specific and remain here.

NB: ao contrário do FordA (binário), aqui os modelos DL usam task="multiclass"
e exigem n_classes (default 8, lido de UWAVE_CONFIG). build_model usa softmax +
sparse_categorical_crossentropy, esperando labels inteiros 0..n_classes-1.
"""
...
# Número de classes default deste experimento (UWaveGestureLibrary tem 8 gestos).
N_CLASSES = 8
...
# ---------------------------------------------------------------------------
# DL models — thin wrappers around base_models.build_model (task=multiclass)
# ---------------------------------------------------------------------------

def create_cnn_model(input_shape: Tuple[int, int], params: Optional[Dict] = None,
                     n_classes: int = N_CLASSES):
    return _build_model("CNN", "raw", input_shape, task="multiclass", n_classes=n_classes, cfg=params)


def create_lstm_model(input_shape: Tuple[int, int], params: Optional[Dict] = None,
                      n_classes: int = N_CLASSES):
    return _build_model("LSTM", "raw", input_shape, task="multiclass", n_classes=n_classes, cfg=params)


def create_cnn_lstm_model(input_shape: Tuple[int, int], params: Optional[Dict] = None,
                          n_classes: int = N_CLASSES):
    return _build_model("CNN_LSTM", "raw", input_shape, task="multiclass", n_classes=n_classes, cfg=params)


def create_transformer_model(input_shape: Tuple[int, int], params: Optional[Dict] = None,
                             n_classes: int = N_CLASSES):
    return _build_model("Transformer", "raw", input_shape, task="multiclass", n_classes=n_classes, cfg=params)
...
def create_learned_wavelet_cnn_model(
    ...
    return _build_model("CNN", mode, input_shape, task="multiclass", n_classes=n_classes, cfg=cfg)
...
def create_learned_wavelet_lstm_model(
    ...
    mode = "learned_wavelet" if cfg.get("warm_start_db4", False) else "learned_wavelet_no_warmup"
    return _build_model("LSTM", mode, input_shape, task="multiclass", n_classes=n_classes, cfg=cfg)
...
def create_learned_wavelet_cnn_lstm_model(
    ...
    mode = "learned_wavelet" if cfg.get("warm_start_db4", False) else "learned_wavelet_no_warmup"
    return _build_model("CNN_LSTM", mode, input_shape, task="multiclass", n_classes=n_classes, cfg=cfg)
...
def create_learned_wavelet_transformer_model(
    ...
    mode = "learned_wavelet" if cfg.get("warm_start_db4", False) else "learned_wavelet_no_warmup"
    return _build_model("Transformer", mode, input_shape, task="multiclass", n_classes=n_classes, cfg=cfg)
