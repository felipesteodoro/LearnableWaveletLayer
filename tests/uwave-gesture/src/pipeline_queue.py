...
(UWaveGestureLibrary — multivariate MULTICLASS classification, 8 gestures, 3 channels).
"""
...
class UWaveExperimentPipeline:
    """
    Loads pre-split UWaveGestureLibrary data, trains one (model_name, mode,
    config_idx) multiclass classifier, and saves metrics.json.
    ...
    def run(self) -> dict:
        ...
        logger.info(
            "Data loaded — train %s  val %s  test %s  input_shape %s  n_classes %d",
            X_train.shape, X_val.shape, X_test.shape, input_shape, self.n_classes,
        )
        ...
        logger.info(
            "Done | %s/%s/cfg%03d — acc=%.4f  f1_macro=%.4f  auc_ovr=%.4f",
            self.model_name, self.mode, self.config_idx,
            metrics["test_accuracy"], metrics["test_f1_macro"], metrics["test_auc_ovr"],
        )
        return metrics
    ...
    def _load_data(self):
        ...
        # Ensure 3-D (batch, time, features). UWave já é multivariado (n, 315, 3),
        # mas o guard mantém compatibilidade caso algum array venha 2-D.
        if X_train.ndim == 2:
    ...
    def _evaluate(self, model, X_train, y_train, X_val, y_val, X_test, y_test, history):
        ...
        def _metrics(y_true, y_proba, prefix):
            # y_proba: (n, n_classes) softmax → argmax para o rótulo predito
            ...
            try:
                out[f"{prefix}_auc_ovr"] = float(
                    roc_auc_score(y_true, y_proba, multi_class="ovr", average="macro", labels=labels)
                )
            except ValueError:
                out[f"{prefix}_auc_ovr"] = float("nan")
            return out

        y_proba_train = model.predict(X_train, verbose=0)
        y_proba_val   = model.predict(X_val,   verbose=0)
